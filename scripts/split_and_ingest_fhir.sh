...
LOCAL_DATA_DIR="synthea/output/fhir"   # Change if your files are elsewhere
...
for file in "$LOCAL_DATA_DIR"/*.json; do
    ...
    echo "Ì≥Ñ $(basename "$file") ‚Äî Detected type: $TYPE"

    if [[ "$TYPE" == "Bundle" ]]; then
        # Extract each resource from the bundle and POST individually
        jq -c '.entry[].resource' "$file" | while read -r resource; do
            RES_TYPE=$(echo "$resource" | jq -r '.resourceType')
            ...
            if [[ "$STATUS" -ge 200 && "$STATUS" -lt 300 ]]; then
                echo "‚úÖ $RES_TYPE from $(basename "$file")" | tee -a "$SUCCESS_LOG"
            else
                echo "‚ùå $RES_TYPE from $(basename "$file") ‚Äî $STATUS" | tee -a "$FAIL_LOG"
            fi
        done
    else
        # Single resource
        STATUS=$(curl -s -o /dev/null -w "%{http_code}" \
          ...
          -H "Content-Type: application/fhir+json" \
          --data-binary @"$file" \
          "https://healthcare.googleapis.com/v1/projects/${PROJECT_ID}/locations/${LOCATION}/datasets/${DATASET_ID}/fhirStores/${FHIR_STORE_ID}/fhir/${TYPE}")
        if [[ "$STATUS" -ge 200 && "$STATUS" -lt 300 ]]; then
            echo "‚úÖ $TYPE from $(basename "$file")" | tee -a "$SUCCESS_LOG"
        else
            echo "‚ùå $TYPE from $(basename "$file") ‚Äî $STATUS" | tee -a "$FAIL_LOG"
        fi
    fi
done
